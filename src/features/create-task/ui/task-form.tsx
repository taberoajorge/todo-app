  ...
  const [state, formAction] = useActionState<FormState, FormData>(
    async (_prevState, formData) => {
      ...
      if (title.length > TASK_LIMITS.TITLE_MAX) {
        return { error: `Title must be ${TASK_LIMITS.TITLE_MAX} characters or less`, success: false };
      }

      if (description && description.length > TASK_LIMITS.DESCRIPTION_MAX) {
        return { error: `Description must be ${TASK_LIMITS.DESCRIPTION_MAX} characters or less`, success: false };
      }
    ...
    <form ref={formRef} action={formAction} className="space-y-4">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span
            className={`text-xs ${
              descLength > TASK_LIMITS.DESCRIPTION_MAX ? 'text-destructive' : 'text-muted-foreground'
            }`}
          >
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          name="deadline"
          type="datetime-local"
          defaultValue={editingTask?.deadline ? toDateTimeLocal(new Date(editingTask.deadline)) : ''}
        />
      </div>
